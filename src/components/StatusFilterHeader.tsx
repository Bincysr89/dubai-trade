...
export default function StatusFilterHeader({ label, options, value, onChange, colorMap }: Props) {
  ...
  return (
    <div className="relative inline-block" ref={ref} style={{ fontFamily: "'Dubai', sans-serif" }}>
      ...
      >
        <span className="text-[14px] text-[#455174] whitespace-nowrap" style={{ letterSpacing: '0.07px', fontWeight: 500 }}>{label}</span>
      ...
      {open && (
        ...
        >
          ...
          >
            <span className="text-[14px] text-[#0e1b3d]">All Statuses</span>
          </button>
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[14px] text-[#0e1b3d]">{opt}</span>
                {active && (
